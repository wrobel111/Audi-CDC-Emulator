...
volatile uint8_t counter_1ms_u8 = 0; // counter for 1ms intervals
volatile uint8_t counter_10ms_u8 = 0; // counter for 10ms intervals
volatile uint8_t flag_50ms = false; // indicates that a period of 50ms isover
#if defined(__AVR_ATmega8__) || defined(__AVR_ATmega128__)
uint8_t counter_timer0_overflows = 0; //timer0 overflow counts to calc 10ms
#endif

volatile bool print_flag = false;
volatile uint8_t my_counter1 = 0;
volatile uint8_t flag_250ms = false;
volatile uint8_t flag_250ms_led = false;
...
void Init_VWCDC(void)
{
  ...
  OCR2A = TIMER2_700US; // for 16MHz 4µs x 175 = 700µs
...
/*
  ...
  Serial.println("Defined timer0 in overflow mode!");
#define _TIMER0_OVERFLOW_COUNTS 50

#else
  OCR0A = TIMER0_10MS; // 10ms Intervall
  ...
  Serial.println("Defined timer0 in CTC mode!");
#endif
*/
  //Timer 3 init
  //Timer 3 used to time the interval between each update
  //Timer3: CTC mode
  TCCR3A = 0;
  TCCR3B = 0;
  TCCR3B |= (1 << WGM32); // CTC mode
  OCR3A = 1249; // For 10 ms: OCR3A = (8 MHz / 64 / 100) - 1 = 1249
  TCCR3B |= (1 << CS31) | (1 << CS30); // Prescaler = 64
  TIMSK3 |= (1 << OCIE3A);             // Enable compare match interrupt

  Serial.println("Defined timer3 in CTC mode!");

  SendPacket(); // force first display update packet
  sei();
  Serial.println("CDC init finish");
...
/*!
  ...
  input capture interrupt (from ICP1 pin) service routine for cdc protocol
  \author     Koelling
  \date       26.09.2007
  \param[in]  none
  \param[out] none
...
#ifndef DISC_TRACK_NUMBER_FROM_MPD
  #ifdef BLUETOOTH
        //remote.handleInterruptFromRadioButton();    //working only in cdc mode, not working if radio is in telephone or fm mode!
  #endif
        disc++;
        if (disc > 0x46)
        {
...

{
  ...
  if (discload >= 0xD1 && discload < 0xD3) {
    discload++;  // przechodzimy przez płyty
  }
  else if (discload == 0xD3) {
    discload = 0x94;  // po płytach przechodzimy do pustych slotów
  }
  else if (discload >= 0x94 && discload < 0x96) {
    discload++;  // przechodzimy przez puste sloty
  }
  else if (discload == 0x96) {
    discload = 0xD1;  // po pustych slotach wracamy do pierwszej płyty
  }
  else {
    discload = 0xD1;  // ustaw domyślnie na pierwszą płytę, jeśli wartość jest nieznana
  }
