...
volatile static uint8_t _inpin = 3;
...
void VAGRadioRemote::setTimer(void){
  ...
  OCR0A = 49;//run compare rutine every 50us  f=20000Hz f=f_clk/(2 x prescaler x (1-OCR0A))
  TCNT0 = 0;
...
void VAGRadioRemote::setTimer(void){
  ...
  OCR1A = 49;//run compare rutine every 50us
  TCNT1 = 0;
...
void VAGRadioRemote::setTimer(void){
  ...
  OCR2A = 49;//run compare rutine every 50us
  TCNT2 = 0;
...
ISR(__TIMERX_COMPA_vect)
{
  ...
  if (captime == 300 ) { //300x50us =>  if 15ms high pulse then reset frame
...
if (sendPtr>0 && counter == 0)
{
  switch (sendPtr)
  {
    case 67: //start bit, 9000us low
      ...
      counter=372;//count form 186 to 0 for ~9000us   186
    break;
    ...
    case 1:
      digitalWrite(_outpin,LOW); //made pin LOW
      counter=22; //each bit start with 600us low     //11
    break;
    //0x41 and 0xE8 data
    case 64://zeroes
    case 60:
...
void VAGRadioRemote::remoteInGoingLow() {
  if (captureEnabled) {
    ...
    if (capptr == 4) {  //let's print what we got
      //Serial.print(capbyte[0],HEX);
      //Serial.print(" ");
      //Serial.print(capbyte[1],HEX);
      //Serial.print(" ");
      //Serial.print(capbyte[2],HEX);
      //Serial.print(" ");
      //Serial.println(capbyte[3],HEX);
    
      if (capbyte[0] == 0x41 && capbyte[1] == 0xE8 && capbyte[3] == 0xFF - capbyte[2]) {
        _newCode = capbyte[2];
        _gotNewCode = 1;
        capptr = 0;
      }
    
    else {  //if frames are wrong, reset all data
    //Serial.println("Frames error");
        capbit = capptr = 0; //start of packet, lets reset it all to begining
        capbyte[0] = capbyte[1] = capbyte[2] = capbyte[3] = 0;
      }
    }
