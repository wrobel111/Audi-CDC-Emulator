...
void BK3254::getInitStates() {
  uint16_t time = 0;
  uint16_t timeout = 500;

  time = millis();
  while (getName() != 1 && time - millis() < timeout);
  time = millis();
  while (getConnectionStatus() != 1 && time - millis() < timeout);
  time = millis();
  while (getPinCode() != 1 && time - millis() < timeout);
  time = millis();
  while (getAddress() != 1 && time - millis() < timeout);
  time = millis();
  while (getMusicStatus() != 1 && time - millis() < timeout);
  time = millis();
  while (getHFPStatus() != 1 && time - millis() < timeout);
}
